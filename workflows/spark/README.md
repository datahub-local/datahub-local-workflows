...
- `/deps/python-deps.pex` with the non-dev Python dependencies needed by Spark Declarative Pipelines.
...
`Dockerfile.bundle` exports the locked runtime requirements from `pyproject.toml` and `uv.lock`, then rebuilds `python-deps.pex` inside the image so dependency wheels are resolved for the target image architecture instead of reusing the host architecture.
