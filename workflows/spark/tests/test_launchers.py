...
class TestResolveSpecPath:
    ...
    def test_resolve_spec_path_accepts_pipeline_relative_path(self):
        """Test resolving a spec path that already includes pipelines/<pipeline>."""
        result = resolve_spec_path("pi", "pipelines/pi/spark-pipeline.yml.j2")
        assert result.exists(), f"Expected spec file not found at {result}"
        assert "pipelines/pi/pipelines/pi" not in str(result)
        assert str(result).endswith("pipelines/pi/spark-pipeline.yml.j2")
