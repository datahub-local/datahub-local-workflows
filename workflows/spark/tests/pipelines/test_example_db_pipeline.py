...
EXAMPLE_DB_SPEC_PATH = EXAMPLE_DB_ROOT / "spark-pipeline.yml.tpl"
...
def test_example_db_pipeline_renders_h2_tables_and_env_overrides(
    ...
    rendered_paths = {
        "source": EXAMPLE_DB_SQL_DIR / "10_automotive_source.sql",
        "raw": EXAMPLE_DB_SQL_DIR / "20_automotive_raw.sql",
        "make": EXAMPLE_DB_SQL_DIR / "30_automotive_make_price_summary.sql",
        "fuel": EXAMPLE_DB_SQL_DIR / "40_automotive_fuel_body_mpg_summary.sql",
    }
    ...
    with spark_pipelines.materialize_spec(
        ...
        source_sql = (library_root / "sql" / "10_automotive_source.sql").read_text()
        raw_sql = (library_root / "sql" / "20_automotive_raw.sql").read_text()
        make_sql = (
            library_root / "sql" / "30_automotive_make_price_summary.sql"
        ).read_text()

        assert rendered_spec["name"] == "datahub-local-workflows-example-db"
        assert rendered_spec["libraries"][0]["glob"]["include"].endswith(
            "/library_0/**"
        )
        assert (
        ...
        assert all(path.exists() for path in rendered_paths.values())
        assert (library_root / "sql" / "40_automotive_fuel_body_mpg_summary.sql").exists()
    ...
    assert all(not path.exists() for path in rendered_paths.values())
...
def test_example_db_aggregation_queries_use_recent_rows_only(
    ...
    with spark_pipelines.materialize_spec(
        ...
        make_sql = (EXAMPLE_DB_SQL_DIR / "30_automotive_make_price_summary.sql").read_text()
        fuel_sql = (
            EXAMPLE_DB_SQL_DIR / "40_automotive_fuel_body_mpg_summary.sql"
        ).read_text()
