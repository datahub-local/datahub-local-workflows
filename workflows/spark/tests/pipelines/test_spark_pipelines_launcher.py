...
def test_launcher_imports_spec_utils_when_loaded_under_unexpected_package() -> None:
    launcher_path = PROJECT_ROOT / "src" / "core" / "launchers" / "spark_pipelines.py"
    spec = importlib.util.spec_from_file_location("pex.spark_pipelines", launcher_path)
    ...
    assert module.__package__ == "pex"
    assert module.DEFAULT_PIPELINE_STORAGE == spark_pipelines.DEFAULT_PIPELINE_STORAGE
    assert module.build_parser().description == "Run the Spark declarative pipeline."
...
def test_materialize_spec_renders_jinja_template_and_overrides_storage(
    ...
    with spark_pipelines.materialize_spec(
        ...
        assert rendered_path.name == "spark-pipeline.yaml"
        assert not (rendered_path.parent / spec_path.name).exists()
...
def test_materialize_spec_renders_imported_sql_templates(tmp_path: Path) -> None:
    ...
    with spark_pipelines.materialize_spec(
        ...
        staged_sql_path = rendered_spec_path.parent / "sql" / "10_template.sql"

        assert not rendered_sql_path.exists()
        assert staged_sql_path.exists()
        assert rendered_spec["libraries"][0]["glob"]["include"] == "sql/**"
        assert not (rendered_spec_path.parent / "sql" / "10_template.sql.j2").exists()
        assert (
...
def test_materialize_spec_renders_sql_templates_with_shared_macros(
    ...
    with spark_pipelines.materialize_spec(
        ...
        assert staged_sql_path.exists()
        assert rendered_spec["libraries"][0]["glob"]["include"] == "sql/**"
        assert not (rendered_spec_path.parent / "lib" / "shared.sql.j2").exists()
        assert (
            staged_sql_path.read_text().strip()
...
def test_materialize_spec_stages_pipeline_files_next_to_rendered_spec(
    tmp_path: Path,
) -> None:
    ...
    write_spec(
        sql_dir / "20_static.sql",
        """
        SELECT 1 AS is_static
        """,
    )
    write_spec(
        common_dir / "pipeline-notes.txt",
        """
    ...
    with spark_pipelines.materialize_spec(
        ...
        rendered_spec = yaml.safe_load(rendered_spec_path.read_text())
        staged_notes_path = rendered_spec_path.parent / "common" / "pipeline-notes.txt"
        staged_sql_path = rendered_spec_path.parent / "sql" / "10_template.sql"
        staged_static_sql_path = rendered_spec_path.parent / "sql" / "20_static.sql"

        assert rendered_spec_path.parent != spec_path.parent
        assert rendered_spec_path.exists()
        assert staged_notes_path.exists()
        assert staged_sql_path.exists()
        assert staged_static_sql_path.exists()
        assert rendered_spec["libraries"][0]["glob"]["include"] == "sql/**"
        assert staged_notes_path.read_text().strip() == "copied without templating"
        assert (
            staged_sql_path.read_text().strip()
            == "SELECT '/tmp/rendered-sql-storage' AS pipeline_storage"
        )
...
def test_main_uses_default_pipeline_storage(monkeypatch, tmp_path: Path) -> None:
    ...
    spark_pipelines.main(["--pipeline", "pi","--spec", str(spec_path), "--dry-run"])
...
def test_main_defaults_spark_api_mode_to_connect(monkeypatch, tmp_path: Path) -> None:
    spec_path = tmp_path / "spark-pipeline.yml"
    write_spec(
        spec_path,
        """
    ...
    def fake_run_pipeline(
        *, spec_path: Path, full_refresh, full_refresh_all, refresh, dry
        ...
        captured["spark_api_mode"] = os.environ.get("SPARK_API_MODE")
        captured["spec_path"] = spec_path

    monkeypatch.delenv("SPARK_API_MODE", raising=False)
    monkeypatch.setattr(spark_pipelines, "run_pipeline", fake_run_pipeline)

    spark_pipelines.main(["--pipeline", "pi", "--spec", str(spec_path), "--dry-run"])

    assert captured["spark_api_mode"] == "connect"
    assert "SPARK_API_MODE" not in os.environ
    assert captured["spec_path"] != spec_path


def test_main_preserves_explicit_spark_api_mode(monkeypatch, tmp_path: Path) -> None:
    spec_path = tmp_path / "spark-pipeline.yml"
    write_spec(
        spec_path,
        """
    ...
    monkeypatch.setenv("SPARK_API_MODE", "classic")
    monkeypatch.setattr(spark_pipelines, "run_pipeline", fake_run_pipeline)

    spark_pipelines.main(["--pipeline", "pi", "--spec", str(spec_path), "--dry-run"])

    assert captured["spark_api_mode"] == "classic"
    assert os.environ["SPARK_API_MODE"] == "classic"
