...
def test_materialize_spec_renders_jinja_template_and_overrides_storage(
    ...
    spec_path = tmp_path / "spark-pipeline.yaml.tpl"
    write_spec(
        spec_path,
        """
...
def test_materialize_spec_renders_imported_sql_templates(tmp_path: Path) -> None:
    ...
    with spark_pipelines.materialize_spec(spec_path, "/tmp/rendered-sql-storage"):
        assert rendered_sql_path.exists()
        assert (
            rendered_sql_path.read_text().strip()
...
def test_materialize_spec_renders_sql_templates_with_shared_macros(
    ...
    write_spec(
        sql_dir / "10_template.sql.tpl",
        """
        {% import 'lib/shared.sql.tpl' as shared %}
