...
def iter_resolved_spec_candidates(pipeline: str, spec_file: str) -> Iterator[Path]:
    source_root = Path(__file__).resolve().parents[2]
    pipelines_root = source_root / "pipelines"
    pipeline_dir = pipelines_root / pipeline
    requested_path = Path(spec_file)
    ...
    if len(requested_path.parts) == 1:
        yield pipeline_dir / requested_path
        return

    yielded: set[Path] = set()
    for candidate in (source_root / requested_path, pipelines_root / requested_path):
...
def resolve_spec_path(pipeline: str, spec_file: str) -> Path:
    ...
    """
    candidates = list(iter_resolved_spec_candidates(pipeline, spec_file))
    ...
    if spec_basename == Path(DEFAULT_SPEC_FILE).stem:
        for variant in SPEC_FILE_VARIANTS:
            for candidate in iter_resolved_spec_candidates(pipeline, variant):
                if candidate.exists():
                    return candidate
