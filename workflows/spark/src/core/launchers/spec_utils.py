...
TEMPLATE_SPEC_SUFFIXES = (".yaml.tpl", ".yml.tpl")
...
def is_template_sql(path: Path) -> bool:
    return path.name.endswith(".sql.tpl")
...
def iter_template_library_includes(include: str) -> Iterator[str]:
    yield include
    if include.endswith(".tpl") or include.endswith("**"):
        return
...
def iter_imported_sql_templates(
    spec_path: Path, spec_data: dict[str, Any]
) -> Iterator[Path]:
    root = spec_path.parent.resolve()
    seen: set[Path] = set()
    for library in spec_data.get("libraries", []):
        ...
        for template_include in iter_template_library_includes(include):
            for path in iter_matching_library_files(root, template_include):
                if is_template_sql(path) and path not in seen:
                    seen.add(path)
                    yield path


def imported_sql_templates(
    spec_path: Path, spec_data: dict[str, Any], pipeline_storage: str
) -> list[Path]:
    rendered_paths: list[Path] = []
    root = spec_path.parent.resolve()
    for template_path in iter_imported_sql_templates(spec_path, spec_data):
        rendered_path = template_path.with_suffix("")
        if rendered_path.exists():
            raise ValueError(
                f"Cannot materialize SQL template {template_path}: {rendered_path} already exists."
            )

        rendered_path.write_text(
            render_template_file(template_path, root=root, pipeline_storage=pipeline_storage)
        )
...
def materialize_runtime_libraries(
    ...
    for index, library in enumerate(spec_data.get("libraries", [])):
        ...
        matched_paths = list(iter_matching_library_files(root, include))
...
def materialize_spec(spec_path: Path, pipeline_storage: str) -> Iterator[Path]:
    ...
    rendered_sql_paths = imported_sql_templates(
    ...
    try:
        ...
        for path in rendered_sql_paths:
