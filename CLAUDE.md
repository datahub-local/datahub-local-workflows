...
- `workflows/dbt/` — dbt Core pipelines on **Trino** (homelab) / **DuckDB** (local), Iceberg + Nessie, medallion architecture; a thin Python `dbt_runner` wraps `dbt build`
...
# Run pipeline (homelab target — Trino/Nessie/S3 required)
...
uv run pytest tests/tasks/test_dlt_utils.py  # single file
```
...
|--------|------------|---------------------|-------------|
| `homelab` (default) | Trino on Kubernetes (Iceberg over Nessie REST + S3) | Iceberg via Nessie REST / Postgres | Production / CI |
...
- **homelab (Trino):** the Trino server defines one Iceberg catalog per Nessie warehouse — `bronze`→`datahub-local-bronze`, `silver`→`datahub-local-silver`, `gold`→`datahub-local-gold`, `test`→`datahub-local-test` (used by `pi`). dbt does not create catalogs.
...
- `dags/tasks/dbt_utils.py` — `DbtTaskConfig` + `create_dbt_task` (dbt image, `python -m dbt_runner`). The shared `build_pod_env_vars` / `build_pod_resources` and the `SecretEnvVarRef`/`ConfigMapEnvVarRef` dataclasses live here. Pod env is just the explicit env/secret/configmap refs — the pods only need to connect to Trino/Postgres/S3.
- `dags/tasks/dlt_utils.py` — `DltTaskConfig` + `create_dlt_task` (dlt image, `python -m dlt_runner`), reusing the dbt_utils builders.
